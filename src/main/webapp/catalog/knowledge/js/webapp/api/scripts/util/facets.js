...
Exhibit.FacetUtilities.constructFlowingFacetItem = function(
    ...
    var classtext = "";
     ...
     var labelWithUnderscore = label.replace(" ", "_");
    
    if (uiContext._collection._database._spo[labelWithUnderscore] != undefined){
    	if (uiContext._collection._database._spo[labelWithUnderscore].inScheme != undefined){
			for (var el = 0; el < uiContext._collection._database._spo[labelWithUnderscore].inScheme.length; el++){
    			if (uiContext._collection._database._spo[labelWithUnderscore].inScheme[el] == "AbstractGeooperatorCategories")
					classtext = " abstractCategory";	
    		}	
    	}
    }
    ...
    var dom = SimileAjax.DOM.createDOMFromString(
        ...
        "<a class='exhibit-flowingFacet-value-link"+classtext+"' href='javascript:{}' id='inner'></a>" +
        " " +
...
Exhibit.FacetUtilities.constructFlowingHierarchicalFacetItem = function(
    ...
    if (hasChildren) {
        dom.showChildren = function(show) {
        	// uncomment following lines to get folded facets  
            // dom.childrenContainer.style.display = show ? "block" : "none";
            // dom.toggle.childNodes[0].style.display = show ? "inline" : "none";
            // dom.toggle.childNodes[1].style.display = show ? "none" : "inline";
            
            // uncomment following lines to get unfolded facets
            dom.childrenContainer.style.display = show ?  "none": "block";
            dom.toggle.childNodes[0].style.display = show ? "none" : "inline";
            dom.toggle.childNodes[1].style.display = show ? "inline" : "none";
        }

...
Exhibit.HierarchicalFacet.prototype.applyRestrictions = function(restrictions) {
	if (this._selections.length - restrictions.length == 1){
		for (var i = 0; i < this._selections.length; i++){
			var inside = false;
			for (var j = 0; j < restrictions.length; j++){
				if (this._selections[i].value == restriction[j].value){
					inside = true;
				}
			}
			if (!inside && document.getElementById(this._selections[i].value) != undefined) {
				document.getElementById(this._selections[i].value).checked = false;
			}
		}
	}
...
Exhibit.HierarchicalFacet.prototype._computeFacet = function(items) {
    ...
    var selectionMap = {};
    var rememberedLinkedSelections;
    for (var i = 0; i < this._selections.length; i++) {
...
//        if (s.value == "Raster to raster" && this._div.id == "f1") {
//            rememberedLinkedSelections = "Map Algebra";
//        }
    }
    if (rememberedLinkedSelections != null) { selectionMap[rememberedLinkedSelections] = false; }
...
Exhibit.HierarchicalFacet.prototype._initializeUI = function() {
    ...
    this._dom = Exhibit.FacetUtilities[this._settings.scroll ? "constructFacetFrame" : "constructFlowingFacetFrame"](
        this._div,
        this._settings.facetLabel, // Reenable here to show facet label in UI. Or put empty string to disable facet label.
...
//        	}
        ...
        var processValue = function(value, nodes, valueSet, path) {
            var label = database.getObject(value, "prefLabel");
            var node = {
                value:  value,

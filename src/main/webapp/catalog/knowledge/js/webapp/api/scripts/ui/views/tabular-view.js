...
Exhibit.TabularView.create = function(configuration, containerElmt, uiContext) {
	var view = new Exhibit.TabularView(containerElmt, Exhibit.UIContext.create(
			configuration, uiContext));
...
Exhibit.TabularView.createFromDOM = function(configElmt, containerElmt,
		uiContext) {
	var configuration = Exhibit.getConfigurationFromDOM(configElmt);

	uiContext = Exhibit.UIContext.createFromDOM(configElmt, uiContext);

	var view = new Exhibit.TabularView(containerElmt != null ? containerElmt
			: configElmt, uiContext);

	Exhibit.SettingsUtilities.collectSettingsFromDOM(configElmt,
			Exhibit.TabularView._settingSpecs, view._settings);
	...
	try {
		...
		for ( var i = 0; i < expressions.length; i++) {
			...
			view._columns
					.push({
						...
						uiContext : Exhibit.UIContext.create({},
								view._uiContext, true),
						styler : null,
		...
		if (formats != null && formats.length > 0) {
			var index = 0;
			var startPosition = 0;
			while (index < view._columns.length
					&& startPosition < formats.length) {
				var column = view._columns[index];
				var o = {};

				column.format = Exhibit.FormatParser.parseSeveral(
						column.uiContext, formats, startPosition, o);
				...
				if (startPosition < formats.length
						&& formats.charAt(startPosition) == ",") {
					startPosition++;
				}
		...
		if (tables.length > 0 && tables[0].rows.length > 0) {
			view._rowTemplate = Exhibit.Lens.compileTemplate(tables[0].rows[0],
					false, uiContext);
		}
	} catch (e) {
		SimileAjax.Debug.exception(e,
				"TabularView: Error processing configuration of tabular view");
	}
		...
		for ( var i = 0; i < columns.length; i++) {
			...
			if (expression.isPath()) {
				...
				if (format != null && format.length > 0) {
					format = Exhibit.FormatParser.parse(view._uiContext,
							format, 0);
				} else {
					format = "list";
				}
...
Exhibit.TabularView.prototype._internalValidate = function() {
	if (this._columns.length == 0) {
		...
		for ( var i = 0; i < propertyIDs.length; i++) {
			...
			if (propertyID != "uri") {
				this._columns.push({
					expression : Exhibit.ExpressionParser.parse("."
							+ propertyID),
					styler : null,
	...
	this._settings.sortColumn = Math.max(0, Math.min(this._settings.sortColumn,
...
Exhibit.TabularView.prototype._initializeUI = function() {
	...
	this._collectionSummaryWidget = Exhibit.CollectionSummaryWidget.create({},
			this._dom.collectionSummaryDiv, this._uiContext);
	if (this._settings.showToolbox) {
		this._toolboxWidget = Exhibit.ToolboxWidget.createFromDOM(this._div,
				this._div, this._uiContext);
...
Exhibit.TabularView.prototype._reconstruct = function() {
	...
	if (items.length > 0) {
		...
		items.sort(this._createSortFunction(items, sortColumn.expression,
		...
		var createColumnHeader = function(i) {
			...
			Exhibit.TabularView.createColumnHeader(exhibit, td, column.label,
					i == self._settings.sortColumn,
					self._settings.sortAscending, function(elmt, evt, target) {
						self._doSort(i);
		...
		if (this._rowTemplate != null) {
			renderItem = function(i) {
				var item = items[i];
				var tr = Exhibit.Lens.constructFromLensTemplate(item.id,
						self._rowTemplate, table, self._uiContext);
            ...
            renderItem = function(i) {
				...
				for ( var c = 0; c < self._columns.length; c++) {
					...
					var valueType = column.format == "list" ? results.valueType
							: column.format;
					
					//DK 
					if (c == 0) {
						column.uiContext.formatLinkList(item.sortKey, item.id,
								function(elmt) { 
									td.appendChild(elmt);
					...
					//END DK
						...
						column.uiContext.formatList(results.values,
								results.size, valueType, function(elmt) {
                                // JB (removes links and renders prefLabel instead of label.
                                if (elmt.nodeName != "A") td.appendChild(elmt);
                                    else  {
                                        elmt.outerHTML = elmt.innerHTML;
                                        td.appendChild(document.createTextNode(database._spo[elmt.innerHTML].prefLabel[0]));
                                    }
                                // END JB
								});
					}
					...
					if (column.label == "Related geooperator(s)") {
						if (td.textContent != "" && td.textContent != undefined) {
							...
							// fallback if only one similar-as element is
							// declared:
							similars.push(td.textContent);

							// if multiple similar-as elements are declared:
							...
							if (td.textContent.indexOf(" und ") != -1) {
								...
								languagesplitter = " und ";
							}
							if (td.textContent.indexOf(" et ") != -1) {
								similars = td.textContent.split(" et ");
								languagesplitter = " et ";
							}
							if (td.textContent.indexOf(" och ") != -1) {
							...
							for (var i = 0; i < similars.length; i++) {
								tdText += "<a href=\"javascript:Exhibit._Impl.prototype._showFocusDialogOnItem('" + database._ops[similars[i]].prefLabel[0] + "')\">" + similars[i] + "</a>";
								tdText += languagesplitter;
							}

							td.innerHTML = tdText.substring(0, tdText.length - languagesplitter.length);
						}
					}
...
Exhibit.TabularView.prototype._getColumnLabel = function(expression) {
	...
	if (property != null) {
		return segment.forward ? property.getLabel() : property
				.getReverseLabel();
	} else {
		return propertyID;
	}
...
Exhibit.TabularView.prototype._createSortFunction = function(items, expression,
		ascending) {
	var database = this._uiContext.getDatabase();
...
Exhibit.TabularView.prototype._doSort = function(columnIndex) {
	...
	var newSortAscending = oldSortColumn == newSortColumn ? !oldSortAscending
			: true;
	...
	SimileAjax.History.addLengthyAction(function() {
		...
		self._reconstruct();
	}, Exhibit.TabularView.l10n.makeSortActionTitle(
			this._columns[columnIndex].label, newSortAscending));
};

Exhibit.TabularView._constructDefaultValueList = function(values, valueType,
		parentElmt, uiContext) {
...
Exhibit.TabularView.createColumnHeader = function(exhibit, th, label, sort,
		sortAscending, sortFunction) {
	...
	if (label == "name") {
		template = {
			elmt : th,
			className : sort ? "exhibit-tabularView-columnHeader-sorted"
	...
	if (sort) {
		template.children
				.push({
					elmt : Exhibit.UI
							.createTranslucentImage(sortAscending ? "images/up-arrow.png"

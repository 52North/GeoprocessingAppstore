...
Exhibit.Collection.prototype.onFacetUpdated = function(facetChanged) {
    if (!this._updating) {
        ...
        this._listeners.fire("onItemsChanged", []);
        if (facetChanged != null && facetChanged._selections != undefined){
	        if (facetChanged._selections.length == 1){
	        	var newName = facetChanged._selections[0].value.replace(/ /g, "_");
	        	if (document.getElementById(newName) != undefined){
	        		document.getElementById(newName).checked = true;
	        		document.getElementById("geodataChoice").style.display= "block";
	        	...
	        	var linksTo = database._spo[facetChanged._selections[0].value].related;
	        	if (linksTo != "" && linksTo != undefined){
	        		for (facet in exhibit._componentMap){
		    			var currentFacet = exhibit._componentMap[facet];
		    			if (currentFacet._cache != undefined){
		    				for (var l = 0; l < linksTo.length; l++){
		    					if (currentFacet._cache.valueToPath[linksTo[l]] != undefined){
		    						if (linksTo[l]!=facetChanged._selections[0].value){
		    							for (var l = 0; l < linksTo.length; l++){
		    	        					links += "<a href=\"javascript:exhibit._componentMap."+facet+".setSelection('" + linksTo[l] + "', true)\">" + database._spo[linksTo[l]].prefLabel + "</a>, ";
		    	        				}
		    						}
		    					}
		    				}
    				...
    				document.getElementById("similarusers").innerHTML = links;
    				document.getElementById("linksToBlock").style.display = "block";
	        	} else {
	        		document.getElementById("similarusers").innerHTML = "";
	        		document.getElementById("linksToBlock").style.display = "none";
	        	}
	        }
        }
    }

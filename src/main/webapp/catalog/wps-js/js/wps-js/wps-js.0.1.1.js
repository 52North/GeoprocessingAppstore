	...
	executeHTTPRequest : function(requestSettings) {
		...
		if (USE_PROXY) {
			if (PROXY_TYPE == "parameter") {
				targetUrl = this.settings.url; //PROXY_URL + encodeURIComponent(this.settings.url); //TODO:bg:proxy stuff disabled - otherwise no results
				
				targetUrl = targetUrl.replace("8080:8080","8080");
				
				console.log("target _ "+targetUrl);
			}
			else {

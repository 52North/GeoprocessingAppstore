...
private void executeCreate(ActionEvent event, RequestContext context) 
  ...
  sCreateSchemaKey="mcp"; //TODO: delete it, if you change back to radio button selection
  setOpenSchemaKey("");
  setOpenDocumentUuid("");
...
/**
 * Use metadata editor for MCP+Upload.
 * @param event event
 * @param context request context
 * @throws Exception if an exception occurs
 */
private void executeMCPEdit(ActionEvent event, RequestContext context)
  throws Exception {
  // edit
  setOpenSchemaKey("");
  setNavigationOutcome("catalog.publication.createMCPMetadata");
}
...
protected void processSubAction(ActionEvent event, RequestContext context) 
  ...
  try {
    if (sCommand.equalsIgnoreCase("create")) {
      ...
      } else if (getOperation().equals("edit")) { //redirect to gxe Editor - no radio buttons needed
    	  setOnBehalfOf(getSelectablePublishers().getSelectedKey());
          executeCreate(event,context); 
    	 // executeEdit(event, context); //activate edit and delete create if you want to use old radio button selection with all schemes
      } 

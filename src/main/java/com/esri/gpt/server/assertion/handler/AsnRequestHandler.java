...
public class AsnRequestHandler {
  ...
  public AsnResponse handleRequest(HttpServletRequest request, HttpServletResponse response) 
    ...
    try {
      ...
      //catch delete assertion earlier - otherwise we dont get assertion no.
      String predicateURN =rOptions.getPredicate();
      String subjectURN = rOptions.getSubject();
      if (predicateURN.contains("delete")){
    	  String[] temp = subjectURN.split(":");
    	  String id = temp[temp.length-1];
    	  id = HttpHelperMethods.getID(id); 
    	  subjectURN = "";temp[temp.length-1]="";
    	  for(String rebuild: temp){
    		  subjectURN+=rebuild+":";
    	  }
    	  subjectURN+=id;
      }
      ...
      //catch create, update and delete operations
      
      if(predicateURN.contains("create") || predicateURN.contains("delete")){
      	HttpHelperMethods.updateDbRatings(subjectURN, true);
      } else if( predicateURN.contains("update")  ){
      	HttpHelperMethods.updateDbRatings(subjectURN, false);
      }    

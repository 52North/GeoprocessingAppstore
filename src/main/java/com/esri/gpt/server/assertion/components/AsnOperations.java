...
public class AsnOperations {
  ...
  /**
   * Makes the operation associated with subject and predicate string
   * @param subject
   * @param predicate
   * @return
   * @throws AsnInvalidOperationException
   */
  public AsnOperation makeOperation(String subjectIn, String predicateIn) 
		    throws AsnInvalidOperationException {
		    AsnOperation operation = null;
		    // determine the the operation based upon the subject and predicate
		    String subjectURN = subjectIn;//"urn:esri:geoportal:resourceid:{C88820AA-A071-4F3D-BCEB-6BAF39DB1635}";//Val.chkStr(context.getRequestOptions().getSubject());
		    String predicateURN = predicateIn;//"urn:esri:geoportal:rating:query";//Val.chkStr(context.getRequestOptions().getPredicate());
		    boolean wasSubjectRecognized = false;
		    boolean wasPredicateRecognized = false;
		    if (subjectURN.length() == 0) {
		      throw new AsnInvalidOperationException("The subject URN was empty.");

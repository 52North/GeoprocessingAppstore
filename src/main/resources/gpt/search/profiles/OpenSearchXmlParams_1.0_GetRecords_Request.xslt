...
Filter: Geooperators 
 -->
  <xsl:template name="filterGeooperators">
    <xsl:variable name="rootGeooperator" select="//filter[@class='com.esri.gpt.catalog.search.SearchFilterGeooperators']"/>
    <xsl:variable name="geooperator" select="normalize-space($rootGeooperator/param[@name='selectedGeooperator']/@value)"/>
    <xsl:if test="contains($searchQueryUrlL, 'geooperator' )">
      <parameter>
        <xsl:attribute name="key">geooperator</xsl:attribute>
        <xsl:attribute name="value"><xsl:value-of select="$geooperator"/></xsl:attribute>
      </parameter>
    </xsl:if>
    <xsl:if test="contains($searchQueryUrlL, '{gpt:geooperator?}' )">
      <parameter>
        <xsl:attribute name="key">{gpt:geooperator?}</xsl:attribute>
        <xsl:attribute name="value"><xsl:value-of select="$geooperator"/></xsl:attribute>
      </parameter>
    </xsl:if>
  </xsl:template>

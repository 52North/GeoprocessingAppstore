...
Filter: Geooperators 
 -->
  <xsl:template name="filterGeooperators" xmlns:ogc="http://www.opengis.net/ogc">
    ...
    <xsl:if test="string-length(normalize-space($geooperator)) > 0">
      <ogc:PropertyIsEqualTo>
        <ogc:PropertyName>geooperator</ogc:PropertyName>
        <ogc:Literal>
          <xsl:value-of select="$geooperator"/>
        </ogc:Literal>
      </ogc:PropertyIsEqualTo>
    </xsl:if>
  </xsl:template>
